...
static int ssat_Command( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    while ( ( c = Extra_UtilGetopt( argc, argv, "PprvSh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'p':
    ...
    // call main function
    ssat_main(pFileName, fReorder, fProjected, fPreprocess, fVerbose);
...
struct ssat_registrar
{
    ssat_registrar() 
    {
        Abc_FrameAddInitializer(&ssat_frame_initializer);
    }
} ssat_registrar_;
