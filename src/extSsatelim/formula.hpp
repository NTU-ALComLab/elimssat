...
class Formula
{
   public:
    ...
    // Modified: Force activate stochastic
    void enforceStochastic(bool val) noexcept;
    // Modified: Edit settings after stochastic is activated
    void setStochastic() noexcept;
    double getElimProb() const noexcept { return _elim_prob; }
    ...
    bool              isStochastic() const noexcept { return _prefix->isStochastic(); }
    void              convertToQBF();
   ...
   private:
    ...
    /**
     * \brief If true, every universal is treated as a random with probability 0.5.
     *  \sa Formula::enforceStochastic(bool)
     */
    bool _enforce_stochastic = false;
    ...
    /**
     * \brief Record the probability of unit random variables
     */
    double _elim_prob;
