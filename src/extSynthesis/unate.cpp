...
void unateDetection(Abc_Ntk_t *, Vec_Int_t *, Vec_Ptr_t *, Vec_Ptr_t *);
...
Abc_Ntk_t* unatePreprocess(Abc_Ntk_t* pNtk, Vec_Int_t **pScope, bool verbose = false) {
  ...
  if (verbose) {
    printf("number of pScope: %d\n", Vec_IntSize(*pScope));
  }
  Vec_Int_t* pScopeNew = Vec_IntAlloc(0);
  unateDetection(pNtk, *pScope, vPosUnate, vNegUnate);
  if (verbose) {
    printf("number of positive unate: %d\n", Vec_PtrSize(vPosUnate));
    printf("number of negative unate: %d\n", Vec_PtrSize(vNegUnate));
  }
  ...
  Vec_IntForEachEntry(*pScope, entry, index) {
    if (!Abc_NodeIsTravIdCurrent(Abc_NtkObj(pNtkNew, entry))) {
...
void unateDetection(Abc_Ntk_t *pNtkCone, Vec_Int_t *pScope,
  ...
  Vec_IntForEachEntry(pScope, entry, index) {
    if (isPositiveUnate(pSatSolver, pCnfPositiveCofactor, pCnfNegativeCofactor,
      ...
      Vec_PtrPush(vPosUnate, Abc_NtkPi(pNtkCone, entry - 1));
    }
    else if (isNegativeUnate(pSatSolver, pCnfPositiveCofactor, pCnfNegativeCofactor,
                        vEnableVars, pMan, entry - 1)) {
      Vec_PtrPush(vNegUnate, Abc_NtkPi(pNtkCone, entry - 1));
    }
  }
  Aig_ManStop(pMan);
