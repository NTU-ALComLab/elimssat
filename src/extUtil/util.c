...
Abc_Ntk_t * Util_NtkExistQuantify( Abc_Ntk_t * pNtk, int pi )
{
    if ( Abc_ObjFanoutNum(Abc_NtkPi(pNtk, pi)) == 0 )
      return pNtk;
    Abc_Ntk_t * pNtkNew = Util_NtkAllocStrashWithName( Abc_ObjName(Abc_NtkCo(pNtk, 0)) );
...
Abc_Ntk_t * Util_NtkExistQuantifyPisByBDD( Abc_Ntk_t * pNtk, Vec_Int_t * pPiIndex )
{
  ...
  int fBddSizeMax = ABC_INFINITY;
  DdManager * dd = (DdManager *)Abc_NtkBuildGlobalBdds(pNtk, fBddSizeMax, 1, fReorder, fReverse, fVerbose) ;
  // Abc_Ntk_t * pNtkNew = Abc_NtkCollapse( pNtk, fBddSizeMax, fDualRail, fReorder, fReverse, fVerbose );
  if (dd == NULL)
  {
    return NULL;
  }
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
void Util_NtkCreatePiFrom( Abc_Ntk_t * pNtkDest, Abc_Ntk_t * pNtkOri, char * pSuffix )
{
    assert( pNtkDest );
    assert( pNtkOri );
    int i;
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
Vec_Ptr_t * Util_AigNtkCounter( Abc_Ntk_t * pNtk, Vec_Ptr_t * pIn, int index_begin, int index_end )
{
    assert(pNtk);
    assert(pIn);
    ...
    Vec_Ptr_t * pSubOut_1 = Util_AigNtkCounter(pNtk, pIn, nSub1_index_start, nSub1_index_end);
    ...
    Vec_Ptr_t * pSubOut_2 = Util_AigNtkCounter(pNtk, pIn, nSub2_index_start, nSub2_index_end);
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
static inline unsigned int Abc_ObjFaninC0_S(Abc_Obj_t * pObj) { return Abc_ObjFaninC0(pObj) ? (~0) : 0; }
static inline unsigned int Abc_ObjFaninC1_S(Abc_Obj_t * pObj) { return Abc_ObjFaninC1(pObj) ? (~0) : 0; }
...
static void printSimulatedResult( Abc_Ntk_t * pNtk )
{
  ...
  {
    Abc_Print( 1, "%d", ( pNode->iTemp != 0 ) );
  }
  Abc_Print( 1, "\n" );
}

#define Util_NtkForEachCiRevere( pNtk, pCi, i )                                                           \
    for ( i = Abc_NtkCiNum(pNtk) - 1; ((i >= 0)) && (((pCi) = Abc_NtkCi(pNtk, i)), 1); i-- )
...
Abc_Ntk_t * Util_NtkBalance( Abc_Ntk_t * pNtk, int fDelete )
{
    ...
    // int fExor        = 0;
    // int fVerbose     = 0;
    Abc_Ntk_t * pNtkNew = Abc_NtkBalance( pNtk, fDuplicate, fSelective, fUpdateLevel );
    if (pNtkNew && fDelete)
        Abc_NtkDelete(pNtk);
    return pNtkNew;
}
...
Abc_Ntk_t * Util_NtkDc2( Abc_Ntk_t * pNtk, int fDelete )
{
    ...
    int fFanout      = 1;
    int fPower       = 0;
    Abc_Ntk_t * pNtkNew = Abc_NtkDC2( pNtk, fBalance, fUpdateLevel, fFanout, fPower, fVerbose );
    if (pNtkNew && fDelete)
        Abc_NtkDelete(pNtk);
    return pNtkNew;
}
...
Abc_Ntk_t * Util_NtkIFraig( Abc_Ntk_t * pNtk, int fDelete )
{
    ...
    int fProve       = 0;
    int fVerbose     = 0;
    Abc_Ntk_t * pNtkNew = Abc_NtkIvyFraig( pNtk, nConfLimit, fDoSparse, fProve, 0, fVerbose );
    if (pNtkNew && fDelete)
       Abc_NtkDelete(pNtk);
    return pNtkNew;
}
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
Abc_Obj_t * Util_NtkCreatePoWithName( Abc_Ntk_t * pNtk, char * PoName, char * pSuffix )
{
    Abc_Obj_t * pPo = Abc_NtkCreatePo( pNtk );
...
/**Function*************************************************************
  ...
  SideEffects [Precondition: input with fanout should has valid pCopy reference.]
  SeeAlso     []
***********************************************************************/
Abc_Obj_t * Util_NtkAppend(Abc_Ntk_t * pNtkDes, Abc_Ntk_t * pNtkSource)
{
    Abc_Obj_t * pObj;
    int i;
    Abc_AigForEachAnd( pNtkSource, pObj, i )
        pObj->pCopy = Abc_AigAnd( (Abc_Aig_t *)pNtkDes->pManFunc, Abc_ObjChild0Copy(pObj), Abc_ObjChild1Copy(pObj) );
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
Abc_Obj_t * Util_NtkAppendWithPiOrder( Abc_Ntk_t * pNtkDes, Abc_Ntk_t * pNtkSource, int start_index )
{
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
Abc_Obj_t * Util_AigNtkMaj(  Abc_Ntk_t * pNtk, Abc_Obj_t * p0, Abc_Obj_t * p1, Abc_Obj_t * p2 )
{
    assert(Abc_NtkIsStrash(pNtk));
...
Abc_Obj_t * Util_AigNtkAdderSum(  Abc_Ntk_t * pNtk, Abc_Obj_t * p0, Abc_Obj_t * p1, Abc_Obj_t * c)
{
    assert(Abc_NtkIsStrash(pNtk));
...
Abc_Obj_t * Util_AigNtkAdderCarry(  Abc_Ntk_t * pNtk, Abc_Obj_t * p0, Abc_Obj_t * p1, Abc_Obj_t * c)
{
    assert(Abc_NtkIsStrash(pNtk));
...
Vec_Ptr_t * Util_AigNtkAdder( Abc_Ntk_t * pNtk, Vec_Ptr_t * p1, Vec_Ptr_t * p2 )
{
    ...
    {
        Abc_Obj_t * pObjIn_1 = (i < Vec_PtrSize(p1)) ? Abc_AigConst0(pNtk) : Vec_PtrEntry(p1, i);
        Abc_Obj_t * pObjIn_2 = (i < Vec_PtrSize(p2)) ? Abc_AigConst0(pNtk) : Vec_PtrEntry(p2, i);

        Abc_Obj_t * pObjSum = Util_AigNtkAdderSum( pNtk, pObjIn_1, pObjIn_2, pObjCarry);
        Vec_PtrPush( pOut, pObjSum);
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
Abc_Obj_t * Util_AigMaj(  Abc_Aig_t * pMan, Abc_Obj_t * p0, Abc_Obj_t * p1, Abc_Obj_t * p2 )
{
...
void Util_ObjPrint( Abc_Obj_t * pObj )
{
  ...
  Abc_Print( 1, "%c%3d: %s\n", Abc_ObjIsComplement(pObj)?'-':' ', Abc_ObjId( pObjReg ), OBJTYPE[ Abc_ObjType(pObjReg) ] );
  if (Abc_ObjFaninNum(pObjReg))
  {
    ...
    {
      Abc_Print( 1, " %c%d", (i<2 && Abc_ObjFaninC(pObjReg, i))?'-':' ' , Abc_ObjFaninId(pObjReg, i));
    }
    Abc_Print( 1, "\n");
  }
}
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
void Util_CollectPi( Vec_Ptr_t * vPi, Abc_Ntk_t * pNtk, int index_start, int index_end )
{
    assert( vPi );
    assert( pNtk );
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
int sat_solver_add_dual_conditional_equal( sat_solver * pSat, int iVar, int iVar2, int iVarCond1, int iVarCond2 )
{
    lit Lits[4];
    int Cid;
    assert( iVar >= 0 );
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
int sat_solver_add_conditional_equal( sat_solver * pSat, int iVar, int iVar2, int iVarCond )
{
    lit Lits[3];
    int Cid;
    assert( iVar >= 0 );
...
/**Function*************************************************************
  ...
  SeeAlso     []
***********************************************************************/
int sat_solver_add_conditional_nonequal( sat_solver * pSat, int iVar, int iVar2, int iVarCond )
{
    lit Lits[3];
    int Cid;
    assert( iVar >= 0 );
...
void Vec_IntPtrFree( Vec_Ptr_t * vVecInts )
{
    int i;
    Vec_Int_t * p;
    Vec_PtrForEachEntry( Vec_Int_t *, vVecInts, p, i )
    {
        Vec_IntFree(p);
    }
